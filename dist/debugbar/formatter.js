...
// ─── Format request list ──────────────────────────────────────────────────
export function formatRequestList(requests) {
    ...
    requests.forEach((req, idx) => {
        ...
        lines.push(`${String(idx + 1).padStart(3)}. ${badge} [${req.id}]`, `     ${req.method.padEnd(7)} ${req.uri}`, `     ${req.datetime} | IP: ${req.ip} | src: ${req.source}`);
        if (req.data) {
            const d = req.data;
            const parts = [];
            if (d.pdo?.nb_statements)
                parts.push(`${d.pdo.nb_statements} SQL`);
            if (d.messages?.count)
                parts.push(`${d.messages.count} log`);
            if (d.exceptions?.count)
                parts.push(`${d.exceptions.count} exc`);
            if (d.time?.duration_str)
                parts.push(`⏱ ${d.time.duration_str}`);
            if (d.memory?.peak_usage_str)
                parts.push(`🧠 ${d.memory.peak_usage_str}`);
            if (parts.length)
...
export function formatRequestSummary(req) {
    ...
    if (d) {
        const collectors = [];
        if (d.pdo)
            collectors.push(['SQL Queries', `${d.pdo.nb_statements} (${d.pdo.accumulated_duration_str})`]);
        if (d.messages)
            collectors.push(['Messages/Logs', String(d.messages.count)]);
        if (d.exceptions)
            collectors.push(['Exceptions', String(d.exceptions.count)]);
        if (d.views)
            collectors.push(['Views', String(d.views.nb_templates)]);
        if (d.events)
            collectors.push(['Events', String(d.events.count)]);
        if (d.models)
            collectors.push(['Models', String(d.models.count)]);
        if (d.cache)
            collectors.push(['Cache', String(d.cache.count)]);
        if (d.auth) {
...
// ─── Format SQL queries ────────────────────────────────────────────────────
export function formatQueries(data, limit = 50) {
    const pdo = data.pdo;
    if (!pdo)
        return 'No PDO/database collector data available for this request.';
    const lines = [
        HR,
    ...
    stmts.forEach((stmt, idx) => {
        ...
        if (stmt.connection)
            lines.push(`     Connection: ${stmt.connection}`);
...
export function formatDuplicateQueries(data) {
    const pdo = data.pdo;
...
export function formatLogs(data, levelFilter) {
    const msgs = data.messages;
    if (!msgs)
        return 'No messages/logs collector data available.';
...
// ─── Format exceptions ────────────────────────────────────────────────────
export function formatExceptions(data) {
    const exc = data.exceptions;
    if (!exc)
        return 'No exceptions collector data available.';
    if (exc.exceptions.length === 0)
        return '✓ No exceptions recorded for this request.';
    const lines = [
        HR,
...
export function formatTimeline(data) {
    ...
    if (!time)
        return 'No timeline/time collector data available.';
    const lines = [
        HR,
...
export function formatRoute(data) {
    ...
    if (!route)
        return 'No route collector data (requires Laravel Debugbar).';
    ...
    if (route.as)
        lines.push(`Name       : ${route.as}`);
    if (route.prefix)
        lines.push(`Prefix     : ${route.prefix}`);
    if (route.namespace)
        lines.push(`Namespace  : ${route.namespace}`);
    if (route.middleware?.length)
        lines.push(`Middleware : ${route.middleware.join(', ')}`);
    if (route.file)
        lines.push(`File       : ${route.file}:${route.line ?? ''}`);
...
export function formatViews(data) {
    const views = data.views;
    if (!views)
        return 'No views collector data (requires Laravel Debugbar).';
    if (views.nb_templates === 0)
        return 'No views rendered for this request.';
    const lines = [
        HR,
    ...
    views.templates.forEach((v, idx) => {
        lines.push(`${idx + 1}. ${v.name} (rendered: ${v.rendered}×)`);
        if (v.type)
            lines.push(`   Type: ${v.type}`);
        if (v.param_count !== undefined)
            lines.push(`   Variables: ${v.param_count}`);
    });
    return lines.join('\n');
}
...
export function formatEvents(data) {
    ...
    if (!events)
        return 'No events collector data (requires Laravel Debugbar).';
    if (events.count === 0)
        return 'No events fired for this request.';
    const lines = [
        HR,
...
// ─── Format auth (Laravel) ────────────────────────────────────────────────
export function formatAuth(data) {
    const auth = data.auth;
    if (!auth)
        return 'No auth collector data (requires Laravel Debugbar).';
    const lines = [
        HR,
        'AUTHENTICATION',
        HR,
...
export function formatModels(data) {
    const models = data.models;
    if (!models)
        return 'No models collector data (requires Laravel Debugbar with collect_models).';
    if (models.count === 0)
        return 'No Eloquent models loaded for this request.';
    const lines = [
        HR,
...
export function formatCache(data) {
    const cache = data.cache;
    if (!cache)
        return 'No cache collector data (requires Laravel Debugbar).';
    if (cache.count === 0)
        return 'No cache operations for this request.';
    const lines = [
        HR,
    ...
    cache.measures.forEach((m, idx) => {
        ...
        if (m.caller)
            lines.push(`   Called  : ${m.caller}`);
    });
    return lines.join('\n');
}
// ─── Format session (Laravel) ─────────────────────────────────────────────
export function formatSession(data) {
    const session = data.session;
    if (!session)
        return 'No session collector data (requires Laravel Debugbar).';
    const lines = [
        HR,
        'SESSION DATA',
        HR,
...
export function formatFullReport(req) {
    if (!req.data) {

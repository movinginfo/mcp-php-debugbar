...
import { PREVIEW_MODES, getLaunchCommands, startPolling, startWebhookReceiver, getInjectableScript, getPhpInjectSnippet, } from '../cursor/preview.js';
...
// ─── Tool registrations ───────────────────────────────────────────────────
export function registerCursorTools(server) {
    // ── debugbar_open_preview ────────────────────────────────────────────
    server.tool('debugbar_open_preview', 'Get the exact commands and steps to open a browser preview with PHP DebugBar integration in Cursor IDE or Chrome. Shows how to enable automatic debug data capture.', {
    ...
    server.tool('debugbar_start_polling', 'Start polling the PHP DebugBar open handler for new requests. Use this when Chrome CDP is not available (e.g. Cursor Simple Browser mode). Every new request is captured automatically.', {
    ...
    server.tool('debugbar_stop_polling', 'Stop the request polling loop.', {}, async () => {
        if (!pollingManager?.isRunning) {
    ...
    server.tool('debugbar_auto_analyze', 'Deeply analyze a captured request with the smart issue detection engine. Returns a health score, detected issues (N+1 queries, slow queries, exceptions, memory, performance), and concrete fix suggestions for each issue.', {
    ...
    server.tool('debugbar_analyze_all', 'Run auto-analysis across ALL cached requests and show a prioritized list of issues, ranked by severity. Perfect for identifying the most critical problems across your entire app.', {
        ...
        const allIssues = results.flatMap(r => r.issues
            ...
            .map(i => ({ ...i, requestUri: r.uri, requestId: r.requestId, score: r.score }))).sort((a, b) => (severityOrder[a.severity] ?? 99) - (severityOrder[b.severity] ?? 99));
    ...
    server.tool('debugbar_suggest_fixes', 'Get a prioritized, actionable list of code fixes for a request based on its debug data. Returns specific PHP/Laravel code suggestions the AI can apply directly.', {
        id: z.string().optional()
    ...
    server.tool('debugbar_watch_and_debug', 'Start continuous monitoring: poll for new requests AND automatically analyze each one. Shows a live debug dashboard in the MCP output. Best used with Cursor Simple Browser.', {
        interval_ms: z.number().optional()
        ...
        const severityOrder = { critical: 0, warning: 1, all: 2 };
    ...
    server.tool('debugbar_webhook_start', 'Start a local webhook server that receives PHP DebugBar IDs sent from an injected JavaScript snippet. Returns the JS snippet and PHP snippet to add to your project for zero-configuration capture.', {
    ...
    server.tool('debugbar_preview_modes', 'Show all available preview modes for Cursor + PHP DebugBar integration, with pros/cons for each approach.', {}, async () => {
        const lines = ['═'.repeat(64), 'AVAILABLE PREVIEW MODES', '═'.repeat(64), ''];

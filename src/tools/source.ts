 ...
 */
function resolveSourceFile(filePath: string, projectRoot: string): string | null {
...
function readFileContext(
  ...
  targetLine: number,
  contextLines = 10,
): { lines: string[]; startLine: number; content: string } | null {
  try {
 ...
 */
function readFullFile(filePath: string, maxLines = 300): string | null {
  try {
    const raw = fs.readFileSync(filePath, 'utf-8');
...
export function registerSourceTools(server: McpServer): void {
  ...
  server.tool(
    ...
    async ({ file, line, context = 15, full = false }) => {
      ...
      const HR = '═'.repeat(60);
      const lines: string[] = [HR, `FILE: ${resolved}`, HR, ''];
  ...
  server.tool(
    ...
    async ({ url, id, all = false, refresh = true }) => {
      ...
      if (req.data?.messages?.count) {
        ...
        lines.push('');
        const labelIcon: Record<string, string> = { error: '🔴', warning: '🟡', info: '🔵', debug: '⚪' };

...
export function registerQueryTools(server) {
    ...
    server.tool('debugbar_get_queries', 'Get all SQL queries executed during a PHP/Laravel request. Shows query text, bindings, duration, row count, and slow queries warning.', {
        id: z
            .string()
            .optional()
        ...
        if ((slow_only || connection || search) && data.pdo) {
            ...
            if (connection) {
                stmts = stmts.filter(s => s.connection?.toLowerCase().includes(connection.toLowerCase()));
            }
            if (search) {
                stmts = stmts.filter(s => s.sql.toLowerCase().includes(search.toLowerCase()));
            }
            data = {
                ...data,
                pdo: {
    ...
    server.tool('debugbar_get_duplicate_queries', 'Detect duplicate/redundant SQL queries in a request — a common performance issue in Laravel (N+1 problem).', {
    ...
    server.tool('debugbar_compare_queries', 'Compare SQL query counts and total time between two captured requests to measure optimization impact.', {
        ...
        // Load data if needed
        for (const [req, setter] of [[req1, (r) => { req1 = r; }], [req2, (r) => { req2 = r; }]]) {
            if (req && !req.data) {
                try {

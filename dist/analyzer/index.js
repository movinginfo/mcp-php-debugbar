...
export function analyzeRequest(req, config = {}) {
    if (!req.data)
        return null;
...
export function analyzeAll(requests, config = {}) {
    return requests
        .filter(r => r.data)
...
// ─── N+1 Detector ─────────────────────────────────────────────────────────
function detectNPlusOne(data, issues, cfg) {
    ...
    for (const [normalized, { count, stmts }] of groups) {
        if (count < cfg.duplicateMinCount)
            continue;
        const stmt = stmts[0];
...
function buildN1Suggestion(table, sql, count, file) {
...
// ─── Slow Query Detector ───────────────────────────────────────────────────
function detectSlowQueries(data, issues, cfg) {
    if (!data.pdo?.statements?.length)
        return;
    for (const stmt of data.pdo.statements) {
        if (stmt.duration < cfg.slowQueryMs)
            continue;
        const severity = stmt.duration >= cfg.criticalQueryMs ? 'critical' : 'warning';
        const file = stmt.backtrace?.[0]?.file;
        const line = stmt.backtrace?.[0]?.line;
        issues.push({
            severity,
...
function buildSlowQuerySuggestion(stmt) {
...
function detectFailedQueries(data, issues) {
    if (!data.pdo?.statements?.length)
        return;
    for (const stmt of data.pdo.statements) {
        if (stmt.is_success)
...
// ─── Exception Detector ───────────────────────────────────────────────────
function detectExceptions(data, issues) {
...
function buildExceptionSuggestion(exc) {
    ...
    const location = appFrame
        ...
        : `${exc.file}:${exc.line}`;
    const lines = [`Fix location: ${location}`];
...
// ─── Memory Detector ──────────────────────────────────────────────────────
function detectMemory(data, issues, cfg) {
    if (!data.memory)
        return;
    const mb = data.memory.peak_usage / 1024 / 1024;
    if (mb < cfg.memoryWarningMb)
        return;
    const severity = mb >= cfg.memoryCriticalMb ? 'critical' : 'warning';
    issues.push({
        severity,
        category: 'memory',
...
function detectPerformanceBottlenecks(data, issues, cfg) {
    if (!data.time)
        return;
    const totalMs = data.time.duration;
    if (totalMs >= cfg.slowRequestMs) {
        const severity = totalMs >= cfg.criticalRequestMs ? 'critical' : 'warning';
...
function buildPerfSuggestion(top, sorted, data) {
    const lines = [];
    if (top) {
...
// ─── Scoring ──────────────────────────────────────────────────────────────
function computeScore(issues, data, cfg) {
    let score = 100;
...
function countDuplicates(stmts) {
    const seen = new Map();

...
let chromeMonitorInstance: ChromeMonitor | null = null;
...
export function registerConnectionTools(server: McpServer): void {
  server.tool(
    'debugbar_connect',
    'Connect to a running PHP/Laravel development server and optionally start monitoring Chrome browser for automatic debugbar data capture.',
    {
      base_url: z
        .string()
        .describe(
          'Base URL of the PHP/Laravel dev server, e.g. http://localhost:8000',
        ),
      open_handler_path: z
        .string()
        .optional()
        .describe(
          'Path to debugbar open handler. Default: /_debugbar/open (Laravel) or /debugbar/open.php (vanilla PHP)',
        ),
      type: z
      ...
      chrome_port: z
        ...
        .optional()
        .describe(
          'Chrome remote debugging port. Start Chrome with: --remote-debugging-port=9222. Enables automatic request capture.',
        ),
      chrome_host: z
        .string()
        .optional()
    ...
    async ({ base_url, open_handler_path, type, chrome_port, chrome_host, load_recent = true }) => {
      ...
      const cfg = {
        ...
        type: projectType as 'laravel' | 'php',
        timeout: 10000,
      ...
      if (cPort) {
        ...
        try {
          ...
          chromeMonitorInstance = await startChromeMonitor(
            cHost,
            cPort,
            baseUrl,
            async (event) => {
  ...
  server.tool(
    'debugbar_disconnect',
    'Disconnect from the PHP server and stop Chrome monitoring.',
    {},
    async () => {
      if (chromeMonitorInstance) {
        await chromeMonitorInstance.stop();
  ...
  server.tool(
    'debugbar_status',
    'Show current connection status, Chrome monitoring state, and number of captured requests.',
    {},
    async () => {
  ...
  server.tool(
    'debugbar_chrome_tabs',
    'List open tabs in Chrome browser (requires Chrome remote debugging port).',
    {
  ...
  server.tool(
    'debugbar_start_chrome_monitor',
    'Start or restart Chrome DevTools Protocol monitoring for automatic debugbar request capture.',
    {
      chrome_port: z.number().optional().describe('Chrome debugging port. Default: 9222'),
      chrome_host: z.string().optional().describe('Chrome host. Default: localhost'),
    },

...
export function getLaunchCommands(url, cdpPort = 9222, browser = 'chrome') {
...
export function startPolling(intervalMs = 2000, onNewRequest) {
    ...
    const poll = async () => {
        if (!running || !state.connectionConfig)
            return;
        try {
...
export async function startWebhookReceiver(port = 9223, onCapture) {
...
export function getInjectableScript(webhookPort = 9223) {
    return `
...
export function getPhpInjectSnippet(webhookPort = 9223) {
    return `<?php

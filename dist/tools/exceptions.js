...
export function registerExceptionTools(server) {
    server.tool('debugbar_get_exceptions', 'Get all exceptions and errors thrown during a PHP/Laravel request, including full stack traces with file and line numbers.', {
        id: z
            .string()
            .optional()
        ...
        if (all_requests) {
            ...
            const withExceptions = state.requests.filter(r => r.data?.exceptions && r.data.exceptions.count > 0);
            if (withExceptions.length === 0) {
                ...
                for (const req of withExceptions) {
                    ...
                    lines.push(`[${req.id.slice(0, 12)}…] ${req.method} ${req.uri} — ${exc.count} exception(s)`);

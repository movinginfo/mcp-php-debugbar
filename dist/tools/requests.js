...
import { formatRequestList, formatRequestSummary, formatFullReport, } from '../debugbar/formatter.js';
export function registerRequestTools(server) {
    // ─── debugbar_list_requests ─────────────────────────────────────────
    server.tool('debugbar_list_requests', 'List all captured PHP/Laravel debugbar requests, showing method, URI, timing, query count, and log count.', {
        max: z
            .number()
            .optional()
    ...
    server.tool('debugbar_get_request', 'Get detailed debugging information for a specific request by its ID. Shows timing, memory, route, queries summary, and more.', {
        id: z
            .string()
            .optional()
    ...
    // ─── debugbar_fetch_url ─────────────────────────────────────────────
    server.tool('debugbar_fetch_url', 'Make an HTTP request to the connected PHP server, capture the debugbar data from the response, and store it. Great for testing specific endpoints.', {
        path: z
            .string()
        ...
        try {
            ...
            if (result.data) {
                ...
                if (d.pdo)
                    lines.push(`   SQL queries: ${d.pdo.nb_statements} (${d.pdo.accumulated_duration_str})`);
                if (d.exceptions?.count)
                    lines.push(`   ⚠️  Exceptions: ${d.exceptions.count}`);
                if (d.messages?.count)
                    lines.push(`   Log messages: ${d.messages.count}`);
            }
            lines.push('');
    ...
    // ─── debugbar_refresh ───────────────────────────────────────────────
    server.tool('debugbar_refresh_requests', 'Fetch the latest requests from the PHP server\'s debugbar storage and update the local cache.', {
    ...
    // ─── debugbar_clear ─────────────────────────────────────────────────
    server.tool('debugbar_clear', 'Clear all locally cached debugbar requests from memory.', {}, async () => {
        state.clearRequests();
        return {
